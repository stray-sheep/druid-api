...
exports.list = function(req, res, next) {
  ...
  Queue.find({
    $and: [
      { $or: [ {"user":req.user._id}, {"organization": {$in: req.user.member}} ] },
      { "archived": false },
    ]},
    function (err, queues) {
    if (err) { console.log(err) }
...
exports.del = function(req, res, next) {
  Queue.findByIdAndUpdate( req.params.id, { archived: true }, function (err, queue) {
    if (err) { console.log(err) }
    else {
      if (queue) { res.json({ success: true, message: 'Queue archived.' }); }
      else { res.status(404); res.json({ success: false, message: 'Queue not found.' }); }
    }
  });
};
